...
Censo_munic |>
  select(ANO, UF, Codmun7, Município, IDHM, IDHM_E, IDHM_L, IDHM_R, POP, pesotot, pesourb, pesoRUR, E_ANOSESTUDO) %>%
...
m1_censo_munic |>
  filter(ANO == 2010) -> censo2_2010
m1_regist_munic |>
  filter(ANO == 2013) -> registro2_2013

# juntar tabelas pela chave do código do município de sete dígitos do IBGE
censo2_2010 |>
  full_join(registro2_2013, by = c("Codmun7"="IBGE7")) |>
